...
/* ── The union cross-test: the cleanest possible version ────────── */
function UnionStudyCard() {
  const study = window.OPTIONS_DATA && window.OPTIONS_DATA.union_study;
  if (!study) return null;
  const negatives = Object.values(study.per_symbol_net).filter(net => net < 0).length;
  const ranked = Object.entries(study.per_symbol_net).sort((a, b) => a[1] - b[1]);
  const money = (value) => `${value < 0 ? "−" : "+"}₹${Math.abs(value).toLocaleString("en-IN")}`;
  return (
    <div className="card">
      <span className="t-eyebrow">The cleanest possible version — tested</span>
      <p style={{ color: "var(--ink-2)", fontSize: 14, maxWidth: 820 }}>
        The natural rebuttal to everything above: <i>"fine, but do it properly — ONLY the biggest stocks, skip every
        results month, skip elections and Budgets."</i> We ran exactly that: the 20 largest F&O names plus all 13
        spreadsheet stocks ({study.n_symbols} large caps, 7 overlap), macro blackouts and earnings months excluded,
        nothing mid- or small-cap anywhere near it. It is the worst configuration in this entire study:
      </p>
      <div style={{ display: "flex", gap: 28, flexWrap: "wrap", margin: "10px 0 12px" }}>
        {[["By his rulebook (score ≥75)", `${(study.rules_stats.total_return * 100).toFixed(1)}%`, "var(--sell)",
           `${study.rules_stats.n_trades} trades · profit factor ${study.rules_stats.profit_factor.toFixed(2)} · drawdown ${(study.rules_stats.max_drawdown * 100).toFixed(0)}%`],
          ["Every stock, every month", `${(study.forced_stats.total_return * 100).toFixed(1)}%`, "var(--sell)",
           `${study.forced_stats.n_trades} trades · win rate ${(study.forced_stats.win_rate * 100).toFixed(1)}% · profit factor ${study.forced_stats.profit_factor.toFixed(2)}`],
          ["Statistical verdict", `t = ${study.forced_stats.t_stat_excess.toFixed(2)}`, "var(--sell)",
           "below −2: provably losing, not unlucky"],
          ["Stocks net-negative", `${negatives} of ${study.n_symbols}`, "var(--hold)",
           "a coin flip decides which giant hurts you"],
        ].map(([label, value, tone, note]) => (
          <div key={label}>
            <div className="t-eyebrow">{label}</div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 24, color: tone }}>{value}</div>
            <div style={{ color: "var(--faint)", fontSize: 11, maxWidth: 210 }}>{note}</div>
          </div>
        ))}
      </div>
      <p style={{ color: "var(--muted)", fontSize: 13 }}>
        Worst: {ranked.slice(0, 4).map(([symbol, net]) => `${symbol} ${money(net)}`).join(" · ")}.
        Best: {ranked.slice(-3).reverse().map(([symbol, net]) => `${symbol} ${money(net)}`).join(" · ")} —
        the best winner earns less than the fourth-worst loser gives back. Making the universe "safer" trims the
        rent but not the gaps: 82% of months pay small, and the same storms still collect. Browse these trades
        below under "The 26-stock clean test".
      </p>
    </div>
  );
}
...
function StockExplorer() {
  ...
  const tradesBySymbol = !data ? {} :
    (dataset === "sheet" && data.sheet_study ? data.sheet_study.trades_by_symbol
      ...
      : dataset === "union" && data.union_study ? data.union_study.trades_by_symbol
  ...
  return (
    <div className="card" style={{ background: "var(--surface)" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        {data.union_study && datasetButton("union", "The 26-stock clean test (top-20 + his 13)")}
      </div>
