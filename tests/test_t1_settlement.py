...
def test_cmd_execute_sell_proceeds_unavailable_same_day(tmp_path: Path):
    ...
    # The mark-to-market call still gets cash + pending so equity is preserved
    cash_passed = mock_mark.call_args.args[1]
    assert abs(cash_passed - 100_000.0) < 0.01  # cash 0 + pending 100k
